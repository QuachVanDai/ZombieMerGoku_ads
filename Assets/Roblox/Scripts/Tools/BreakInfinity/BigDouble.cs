    ...
    public struct BigDouble : IFormattable, IComparable, IComparable<BigDouble>, IEquatable<BigDouble>
    {
        ...
        public static BigDouble Parse(string value)
        {
            if (value.IndexOf('e') != -1)
            {
                ...
                var exponent = long.Parse(parts[1]);
            ...
            var result = new BigDouble(double.Parse(value));
            if (IsNaN(result))
            {
        ...
        public static string FormatShorthand(BigDouble value)
        {
            if (IsNaN(value))
                return "NaN";

            if (IsInfinity(value))
                return IsPositiveInfinity(value) ? "Infinity" : "-Infinity";

            if (IsZero(value.Mantissa))
                return "0";

            // If over 1000U, use default formatting
            var absValue = Abs(value);
            if (absValue.Exponent >= 39)
        ...
        {
                (exponent: 36L, abbreviation: "U"),   // Undecillion
                (exponent: 33L, abbreviation: "D"),   // Decillion
                (exponent: 30L, abbreviation: "N"),   // Nonillion
                (exponent: 27L, abbreviation: "O"),   // Octillion
                (exponent: 24L, abbreviation: "S"),   // Septillion
                (exponent: 21L, abbreviation: "Sx"),  // Sextillion
                (exponent: 18L, abbreviation: "Qn"),  // Quintillion
                (exponent: 15L, abbreviation: "Qa"),  // Quadrillion
                (exponent: 12L, abbreviation: "T"),   // Trillion
                (exponent: 9L, abbreviation: "B"),    // Billion
                (exponent: 6L, abbreviation: "M"),    // Million
                (exponent: 3L, abbreviation: "K"),    // Thousand
            };

            foreach (var (exponent, abbreviation) in abbreviations)
            {
                if (absValue.Exponent >= exponent)
                {
                    // Shift the value down to 0-3 range for display
                    var displayValue = absValue * BigDouble.Pow10(-exponent);
                    var mantissaStr = displayValue.ToDouble().ToString("F2").TrimEnd('0').TrimEnd('.');

                    // Apply sign if negative
                    if (value.Mantissa < 0)
                        mantissaStr = "-" + mantissaStr;

                    return $"{mantissaStr}{abbreviation}";
                }
            }

            // For values less than 1000, return the regular format
            return value.ToString("G3");
        }
        ...
        private static class BigNumber
        {
            ...
            private static string FormatGeneral(BigDouble value, int places)
            {
                ...
                {
                    return value.ToDouble().ToString(format);
                }

                return value.Mantissa.ToString(format)
                       + "E" + (value.Exponent >= 0 ? "+" : "")
        ...
        private static class PowersOf10
        {
            ...
            static PowersOf10()
            {
                ...
                {
                    Powers[index++] = double.Parse("1e" + (i - IndexOf0));
                }
            }
