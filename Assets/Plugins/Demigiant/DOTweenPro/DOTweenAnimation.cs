...
namespace DG.Tweening
{
    ...
    public class DOTweenAnimation : ABSAnimationComponent
    {
        ...
        static Vector3 SwitchToRectTransform(RectTransform from, RectTransform to)
        {
            if (from == null || to == null) return Vector3.zero;

            Transform parent = to.parent;
            if (parent == null) return from.position;

            Vector3 localPosition = parent.InverseTransformPoint(from.position);
            return localPosition;
        }

        static Tweener DOAnchorPos3D(RectTransform target, Vector3 endValue, float duration, bool snapping)
        {
            return DOTween.To(() => target.anchoredPosition3D, x => target.anchoredPosition3D = snapping ? RoundVector3(x) : x, endValue, duration)
                .SetTarget(target);
        }

        static Tweener DOSizeDelta(RectTransform target, Vector2 endValue, float duration)
        {
            return DOTween.To(() => target.sizeDelta, x => target.sizeDelta = x, endValue, duration)
                .SetTarget(target);
        }

        static Tweener DOGraphicColor(Graphic target, Color endValue, float duration)
        {
            return DOTween.To(() => target.color, x => target.color = x, endValue, duration)
                .SetTarget(target);
        }

        static Tweener DOGraphicFade(Graphic target, float endValue, float duration)
        {
            return DOTween.ToAlpha(() => target.color, x => target.color = x, endValue, duration)
                .SetTarget(target);
        }

        static Tweener DOCanvasGroupFade(CanvasGroup target, float endValue, float duration)
        {
            return DOTween.To(() => target.alpha, x => target.alpha = x, endValue, duration)
                .SetTarget(target);
        }

        static Tweener DOSpriteColor(SpriteRenderer target, Color endValue, float duration)
        {
            return DOTween.To(() => target.color, x => target.color = x, endValue, duration)
                .SetTarget(target);
        }

        static Tweener DOSpriteFade(SpriteRenderer target, float endValue, float duration)
        {
            return DOTween.ToAlpha(() => target.color, x => target.color = x, endValue, duration)
                .SetTarget(target);
        }

        static Tweener DOText(Text target, string endValue, float duration)
        {
            return DOTween.To(() => target.text, x => target.text = x, endValue, duration)
                .SetTarget(target);
        }

        static Vector3 RoundVector3(Vector3 value)
        {
...
//            }
            ...
            case AnimationType.Move:
                        ...
                        if (targetType == TargetType.RectTransform) {
                            ...
                            if (endValueT == null) {
                                ...
                                if (rTarget == null) {
                                    ...
                                    endValueV3 = SwitchToRectTransform(endValueT, rTarget);
                                }
                            }
                        } else
#endif
...
#if true // UI_MARKER
                    tween = DOAnchorPos3D((RectTransform)target, endValueV3, duration, optionalBool0);
#else
                    tween = ((Transform)target).DOMove(endValueV3, duration, optionalBool0);
#endif
                    break;
                ...
                case TargetType.Rigidbody2D:
                    ...
                    tween = ((Rigidbody2D)target).transform.DOMove(endValueV3, duration, optionalBool0);
#else
                    tween = ((Transform)target).DOMove(endValueV3, duration, optionalBool0);
#endif
                    break;
            ...
            case AnimationType.Rotate:
                ...
                case TargetType.Rigidbody2D:
#if true // PHYSICS2D_MARKER
                    tween = ((Rigidbody2D)target).transform.DORotate(new Vector3(0, 0, endValueFloat), duration, RotateMode.Fast);
#else
                    tween = ((Transform)target).DORotate(endValueV3, duration, optionalRotationMode);
#endif
                    break;
            ...
            case AnimationType.UIWidthHeight:
                tween = DOSizeDelta((RectTransform)target, optionalBool0 ? new Vector2(endValueFloat, endValueFloat) : endValueV2, duration);
                break;
#endif
            case AnimationType.Color:
                ...
                case TargetType.SpriteRenderer:
                    tween = DOSpriteColor((SpriteRenderer)target, endValueColor, duration);
                    break;
#endif
#if true // UI_MARKER
                case TargetType.Image:
                    tween = DOGraphicColor((Graphic)target, endValueColor, duration);
                    break;
                case TargetType.Text:
                    tween = DOGraphicColor((Text)target, endValueColor, duration);
                    break;
#endif
#if false // TK2D_MARKER
                case TargetType.tk2dTextMesh:
...
#if true // SPRITE_MARKER
                case TargetType.SpriteRenderer:
                    tween = DOSpriteFade((SpriteRenderer)target, endValueFloat, duration);
                    break;
#endif
#if true // UI_MARKER
                case TargetType.Image:
                    tween = DOGraphicFade((Graphic)target, endValueFloat, duration);
                    break;
                case TargetType.Text:
                    tween = DOGraphicFade((Text)target, endValueFloat, duration);
                    break;
                case TargetType.CanvasGroup:
                    tween = DOCanvasGroupFade((CanvasGroup)target, endValueFloat, duration);
                    break;
#endif
#if false // TK2D_MARKER
                case TargetType.tk2dTextMesh:
            ...
            case AnimationType.Text:
                ...
                switch (targetType) {
                case TargetType.Text:
                    tween = DOText((Text)target, endValueString, duration);
                    break;
                }
#endif
#if false // TK2D_MARKER
...
#if true // UI_MARKER
                case TargetType.RectTransform:
                    tween = ((RectTransform)target).DOPunchPosition(endValueV3, duration, optionalInt0, optionalFloat0, optionalBool0);
                    break;
#endif
                }
                break;
...
#if true // UI_MARKER
                case TargetType.RectTransform:
                    tween = ((RectTransform)target).DOShakePosition(duration, endValueV3, optionalInt0, optionalFloat0, optionalBool0, optionalBool1);
                    break;
#endif
                }
                break;
